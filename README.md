...
O **Libre Turing** é um Sistema de Gerenciamento de Bibliotecas (TPS) desenvolvido para Web. [cite_start]O objetivo principal foi criar uma interface intuitiva e acessível, focada na experiência do usuário (UX) e na aplicabilidade das heurísticas de usabilidade.

Diferente de sistemas complexos, o Libre Turing foi projetado para reduzir a carga cognitiva, permitindo que tanto bibliotecários experientes quanto usuários leigos operem o sistema com facilidade.

### 🛠️ Arquitetura e Persistência de Dados
Para fins acadêmicos e de prototipagem de interface, o sistema não utiliza um banco de dados relacional tradicional (como MySQL). Em vez disso, utilizamos **JavaScript** e **JSON** para simular a persistência de dados diretamente no **Local Storage** do navegador. Isso torna o projeto leve e executável em qualquer máquina sem necessidade de configuração de backend.
...
* **Affordances:** Botões e elementos interativos possuem design que sugere sua funcionalidade (clicável, editável)[cite: 53].
* **Metáforas Visuais:** Uso de ícones universalmente reconhecidos (ex: Lupa para pesquisa, Lixeira para exclusão) para facilitar o entendimento sem necessidade de muita leitura.
...
* **Saídas de Emergência:** Possibilidade de desfazer navegações e voltar ao menu principal a qualquer momento.
...
Projeto desenvolvido pelos alunos de Sistemas de Informação:
...
**Professor Orientador:** Vinicius Ferreira de Souza
